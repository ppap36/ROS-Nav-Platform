...
class InterpolatorNode(object):
    def __init__(self):
        ...
        #self._path_sub = rospy.Subscriber("/rrt_star_planner/RRTstarPlannerROS/plan", Path, self._accept_path_from_topic, queue_size=1)
        self._path_sub = rospy.Subscriber("/move_base/HybridAStarPlanner/plan", Path, self._accept_path_from_topic, queue_size=1)
    ...
    def continue_path(self, start_time=None):
        # if we're un-paused and we were busy with a section: we Rresume the path from the last goal send:
